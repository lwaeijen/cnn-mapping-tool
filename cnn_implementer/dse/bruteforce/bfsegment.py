...
class BFSegment(DSESegment):

    def __init__(self, **kwargs):

        #Init super
        super(BFSegment, self).__init__(**kwargs)
    ...
    def __config_space(self,
        ...
        inner_loops=['zo_i','y_i','x_i']+( ['zi_i'] if out['type'] in ['convolution'] else [] )
        outer_loops=['zo_o','y_o','x_o']+( ['zi_o'] if out['type'] in ['convolution'] else [] )

        #define the design search space for this segment
        space={
            'Tx':               set([2**p for p in range(int(floor(log(out['xo'],2)))+1)]+[out['xo']]) if not notiling else [out['xo']],
            'Ty':               set([2**p for p in range(int(floor(log(out['yo'],2)))+1)]+[out['yo']]) if not notiling else [out['yo']],
            'Tzo':              set([2**p for p in range(int(floor(log(out['zo'],2)))+1)]+[out['zo']]) if not notiling else [out['zo']],
            'order_inner':      map(lambda t: list(t), permutations(inner_loops)),
            'order_outer':      outer_loops,
        }
        if out['type'] in ['convolution']:
            space['Tzi'] = set([2**p for p in range(int(floor(log(out['zi'],2)))+1)]+[out['zi']/out['groups']]) if not notiling else [out['zi']/out['groups']]
        else:
            space['Tzi'] = [1]
    ...
    def __all_configs_raw(self,
        ...
        for cfg_v in product(*space.values()):
            ...
            #resolve order
            cfg['order']=list(cfg['order_inner']) + [cfg['order_outer']] + filter(lambda lvl: lvl!=cfg['order_outer'], ['zi_o','zo_o','y_o','x_o'])
            del cfg['order_inner']
            del cfg['order_outer']
